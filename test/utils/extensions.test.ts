...
test("transposes a single row matrix into a column", () => {
  const input = [[1, 2, 3]];
...
test("transposes a single column matrix into a row", () => {
  const input = [[1], [2], [3]];
  const expected = [[1, 2, 3]];
...
test("works with string matrices", () => {
  const input = [
    ["a", "b"],
    ["c", "d"],
  ];
...
test("it does not mutate the original matrix", () => {
  const input = [
    [1, 2],
    [3, 4],
  ];
