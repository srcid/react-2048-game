import { Board } from "../../src/utils/gameboard";
...
describe("pushIfNotEqualsTop", () => {
  ...
  const boardA: Board = [[0, 1], [1, 0]];
  const boardB: Board = [[1, 1], [1, 1]];
  ...
  test("should not add the board if it is identical to the current top", () => {
    const history: Board[] = [boardA];
    const sameBoard: Board = [[0, 1], [1, 0]]; // Deeply equal but different reference
